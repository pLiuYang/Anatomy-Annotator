...
void PMeshViewer::newProject()
{
    ...
    /*// TEST vtk button widget
    vtkButtonRepresentation *rep = (static_cast<vtkButtonRepresentation> (vtkButtonRepresentation::New()));
    rep->SetNumberOfStates(2);
    vtkButtonWidget *btn = vtkButtonWidget::New();
    btn->SetInteractor(interactor);
    btn->SetRepresentation(rep);

    vtkCoordinate *ur = vtkCoordinate::New();
    ur->SetCoordinateSystemToNormalizedDisplay();
    ur->SetValue(1.0, 1.0);
    double bds[6];
    double sz = 50.0;
    bds[0] = ur->GetComputedDisplayValue(renderer)[0] - sz;
    bds[1] = bds[0] + sz;
    bds[2] = ur->GetComputedDisplayValue(renderer)[1] - sz;
    bds[3] = bds[2] + sz;
    bds[4] = bds[5] = 0.0;
    btn->On();*/
}
